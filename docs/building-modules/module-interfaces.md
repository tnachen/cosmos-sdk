...
This document details how to build CLI and REST interfaces for a module. Examples from various modules will be included.
...
- **`codec`**. The getter function takes in an application [`codec`](../core/encoding.md) as an argument and returns a reference to the `cobra.command`.
- **Construct the command:** Read the [Cobra Documentation](https://github.com/spf13/cobra) for details on how to create commands.
  + **Use:** Specifies the command format a user types in order to invoke this command. In this case, the user uses `buy-name` as the name of the transaction command and provides the `name` the user wishes to buy and the `amount` the user is willing to pay.
...
- **Flags.** Add any [flags](#flags) to the command. No flags were specified here, but all transaction commands have flags to provide additional information from the user (e.g. amount of fees they are willing to pay).
...
The input provided for this flag - called `FlagGrom` is a string with the default value of `""` if none is provided. If the user asks for a description of this flag, the description will be printed.
